...
class K8sGrafanaProgress(Plugin):
    ...
    def __init__(self, app_id, monitor_plugin, enable_visualizer,
        ...
        # FIXME(rafael): get node ip from k8s api instead of config
    ...
    def _create_grafana_service(self, namespace='default'):
        ...
        Arguments:
            namespace {string} -- Namespace
            that thegrafana service will be created
    ...
    def _create_datasource(self, grafana_user, grafana_password,
        ...
        Arguments:
            grafana_user {string} -- Grafana's user with the
            necessary permissions
            grafana_password {string} -- Password of the Grafana user
            visualizer_ip {string} -- IP of one of the
    ...
    def _create_dashboard(self, grafana_user, grafana_password,
        ...
        Arguments:
            grafana_user {string} -- Grafana's user with
    ...
    def _create_grafana_components(self, app_id, grafana_user,
        ...
        Create a Pod for the visualizer and expose it
        ...
        try:
            datasource_created = \
                self._create_datasource(grafana_user, grafana_password,
                                        visualizer_ip, node_port, timeout)

            dashboard_created = \
                self._create_dashboard(grafana_user, grafana_password,
                                       visualizer_ip, node_port, timeout)
